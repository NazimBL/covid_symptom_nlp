...
# === 3. Create one-hot matrix for symptom categories ===
...
counts = symptom_matrix.sum().values
valid = counts > 0
avg_expr_mat = symptom_matrix.T.dot(df[gene_cols])[valid] / counts[valid, None]
avg_expr_mat.index = symptom_matrix.columns[valid]

# === 6. Save matrix and plot ===
avg_expr_mat.to_csv("average_expression_per_symptom_3-6M.csv")

plt.figure(figsize=(20, 10))
sns.heatmap(
    avg_expr_mat,
    cmap="coolwarm",
    vmin=avg_expr_mat.values.min(),
    vmax=avg_expr_mat.values.max(),
...
plt.tight_layout()
plt.savefig("avg_expression_heatmap_3-6M.png", dpi=300)
plt.show()
